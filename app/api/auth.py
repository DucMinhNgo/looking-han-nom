...
* **everyone else** lives in ``data/users.json`` and is created by an admin.
...
# /img/ is NOT public: nothing external fetches these any more, so they sit
# behind the session like everything else. The browser sends the cookie with
# <img> requests on its own, so this costs nothing.
    ...
    def validate(self) -> None:
        ...
        if missing:
            raise AuthNotConfigured(
                f"Refusing to start: {', '.join(missing)} not set. Run "
                "`python -m app.cli --init-env` to generate a working .env, "
                "or see .env.example."
            )
